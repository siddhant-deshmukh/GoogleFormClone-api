...
}), (0, express_validator_1.body)('correct_ans.*.ans').optional().isString().isLength({ min: 3, max: 50 }), (0, express_validator_1.body)('point').optional().isInt({ max: 100, min: 0 }), (req, res, next) => {
    const errors = (0, express_validator_1.validationResult)(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array() });
    }
